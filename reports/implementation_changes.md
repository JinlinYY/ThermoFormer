...
- Resume checks reject stale Git/config/split/cache/environment fingerprints. Formal aggregation requires exactly seeds 0–4 and rejects duplicate metric scopes, mixed provenance, any missing/changed material artifact and all non-finite metrics. A single atomic aggregate manifest is invalidated before replacement and marked complete only after both tables and their hashes are committed; partial-seed summaries are isolated under `diagnostic_*` names.
