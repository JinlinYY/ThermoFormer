...
- Reorganized `predictive_performance_report.md` by the two actual inference contracts instead of presenting P, T and pooled y as three apparently independent targets. Isothermal P–x–y blocks now report the coupled P/y outputs; isobaric T–x–y blocks report the coupled T/y outputs. Added `*_by_task.csv` tables for overall, state, unseen-chemistry and binary-to-ternary evaluations while retaining the original pooled summaries for compatibility.
