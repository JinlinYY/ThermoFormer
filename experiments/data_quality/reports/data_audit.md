...
The row-level mapping is saved as `experiments/data_quality/reports/ternary_binary_subsystem_coverage.csv`. This dataset-wide table describes availability in the modeling corpus. Every binary-to-ternary experiment independently recomputes coverage from that seed's **actual binary training partition** before stratification and subgroup evaluation.
...
- The legacy CLI generates grouped splits in memory. Paper experiments instead use 75 versioned JSON artifacts under `datasets/splits/vle/`; the runner refuses dataset-digest mismatch, duplicate/unknown IDs, unsafe protocol names and row overlap. The complete audit is in `experiments/data_quality/reports/split_audit.md`/`.json`.
