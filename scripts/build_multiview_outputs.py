...
def main() -> None:
    ...
    if args.stage == "formal":
        ...
        costs = formal_cost_table(PROJECT_ROOT)
        gates = gate_summary_table(PROJECT_ROOT)
        ...
        cost_path = PROJECT_ROOT / "results" / "multiview" / "formal" / "formal_costs.csv"
        gate_path = PROJECT_ROOT / "results" / "multiview" / "analysis" / "multiview_gate_summary.csv"
        atomic_csv(formal_path, formal)
        atomic_csv(seed_path, formal_seeds)
        atomic_csv(cost_path, costs)
        atomic_csv(gate_path, gates)
        outputs.extend([formal_path, seed_path, cost_path, gate_path])
        outputs.extend(write_variant_result_pages(PROJECT_ROOT, screening, formal))
        outputs.append(
            write_final_report(PROJECT_ROOT, screening, formal, formal_seeds, costs, gates)
        )
    else:
        outputs.extend(write_variant_result_pages(PROJECT_ROOT, screening))
